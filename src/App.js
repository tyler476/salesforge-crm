...
(function injectPresentationStyles() {
  ...
  s.textContent = `
    ...
    @keyframes barSlide  { from { width: 0; } to { width: var(--bar-w); } }
    ...
    .bar-grow   { animation: barGrow  .65s cubic-bezier(.34,1.2,.64,1) both; }
    .bar-slide  { animation: barSlide .7s cubic-bezier(.34,1.1,.64,1) both; }
    .fade-up    { animation: fadeUp   .5s ease both; }
    .fade-in    { animation: fadeIn   .4s ease both; }
    .scale-in   { animation: scaleIn  .45s cubic-bezier(.34,1.1,.64,1) both; }
    .count-up   { animation: countUp  .5s cubic-bezier(.34,1.2,.64,1) both; }
    .tip-box    { animation: scaleIn  .15s ease both; transform-origin: bottom left; }
  `;
  document.head.appendChild(s);
})();

// ── Shared loan math ──────────────────────────────────────────────────────────
function calcLoan(principal, annualRate, termYears) {
  const p  = Number(principal)  || 0;
  const r  = Number(annualRate) || 0;
  const y  = Number(termYears)  || 30;
  const mr = r / 100 / 12;
  const n  = y * 12;
  const mp = mr > 0
    ? Math.round(p * mr * Math.pow(1+mr,n) / (Math.pow(1+mr,n)-1))
  ...
  const totalPaid = mp * n;
  const totalInt  = totalPaid - p;
  // Equity after 5 years via amortization formula
  const bal5 = mr > 0
    ? Math.round(p * Math.pow(1+mr,60) - mp * (Math.pow(1+mr,60)-1)/mr)
    : Math.max(0, p - mp*60);
  const equity5 = Math.max(0, p - bal5);
  const breakeven = p > 0 ? Math.round(30 + (p/100000)*6) : 0;
  const fmt = v => v >= 1000000 ? '$'+(v/1000000).toFixed(2)+'M'
               : v >= 1000      ? '$'+v.toLocaleString()
               : '$'+v;
  return { p, r, y, mr, n, mp, totalPaid, totalInt, equity5, breakeven, fmt };
}

function buildSlides(form) {
  const { p, r, y, mp, totalPaid, totalInt, equity5, breakeven, fmt } = calcLoan(form.loan_amount, form.rate, form.term);
  ...
  return [
    ...
    },
    {
      type:'cf-savings',
      metrics:[
        { badge:'Most Interest Savings', label:'Total over '+y+'-year term',   value:totalInt>0?fmt(totalInt):'—',           description:'Total interest paid over the '+y+'-year term at '+r+'% fixed rate' },
        { badge:'Monthly Savings',        label:'vs average renter',            value:mp>0?fmt(Math.round(mp*.06)):'—',       description:'Estimated monthly savings vs continuing to rent in this market' },
        { badge:'Equity by Year 5',       label:'Through principal paydown',    value:equity5>0?fmt(equity5):'—',             description:'Equity accumulated through amortization in the first 5 years' },
      ],
      currentInterest: totalInt>0?fmt(Math.round(totalInt*1.16)):'—',
      newInterest:     totalInt>0?fmt(totalInt):'—',
      savingsPct:      '86',
    ...
    },
    {
      type:'cf-table',
      rows:[
        { label:'Loan Amount',     current:'Existing Loan',  newVal: p>0?fmt(p):'—' },
        { label:'Loan Type',       current:'Current',        newVal: form.loan_type||'—' },
        { label:'Term',            current:'Existing',       newVal: y+' Years' },
        { label:'Interest Rate',   current:'Current Rate',   newVal: r+'%' },
        { label:'Monthly P&I',     current:'—',              newVal: mp>0?fmt(mp):'—',                         highlight:true },
        { label:'Total Monthly',   current:'—',              newVal: mp>0?fmt(Math.round(mp*1.28)):'—',        highlight:true },
        { label:'Total Interest',  current:'—',              newVal: totalInt>0?fmt(totalInt):'—' },
...
// ── ScaledSlide — renders 1280×720 and scales to fill any container ───────────
